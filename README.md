...
```bash
cd client
npm install        # or: npm ci  (clean install from the lockfile)
```

### 2. Set up the database

In the Supabase dashboard, open **SQL Editor → New query**, paste the contents
of [`client/supabase-schema.sql`](./client/supabase-schema.sql), and run it. This
creates the `patient_profiles` and `admissions` tables, row-level security
policies, and enables realtime on `admissions`.

### 3. Configure environment variables

Copy the frontend template and fill in your project values:
